...
class Main:
    def __init__(self):
        ...
        if self.threads == 0:
            self.threads = 1
        proxylist = Checker.Proxy.proxylist
        try:
            self.proxys = open(proxylist, 'r', encoding='u8', errors='ignore').read().split()
        except FileNotFoundError:
            print(f'{Fore.LIGHTRED_EX}{proxylist} not found, Please make sure {proxylist} is in folder')
...
\n'''
        ...
        pool.join()
        while True:
            if self.printing.qsize() == 0 and len(
                    self.towrite) == 0 and self.caputer.qsize() == 0 and self.bad.qsize() == 0 and self.hits.qsize() == 0:
                sleep(4)
        ...
        try:
            ...
            if str(answer).__contains__("errorMessage"):
                ...
                if str(answer).__contains__("'legacy': True"):
                    ...
                    self.towrite.append([line, 'Unmigrated'])
                    data += '\nUnmigrated: True'
                    sfa = False
                ...
                if securec == '[]':
                    ...
                    self.towrite.append([line, 'Unsecured'])
                    data += '\nUnsecured: True'
                elif securec == 'False':
                    pass
                else:
                    ...
                    self.towrite.append([line, 'Secured'])

                if self.name(username):
                    Counter.special_name += 1
                    self.towrite.append([f'{line} | Username: {username}', 'SpecialName'])
                    data += '\nSpecialName: True'
                ...
                if self.hypr or self.hypl:
                    ...
                    if self.hypr:
                        if hp[0] != 'False':
                            ...
                            self.towrite.append([f'{line} | Rank: {hp[0]}', 'HypixelRanked'])
                            data += f'\nHypixelRank: {hp[0]}'
                    ...
                    if self.hypl:
                        if int(hp[1]) != 0:
                            ...
                            if int(hp[1]) >= self.hypminl:
                                Counter.hypixelhl += 1
                                self.towrite.append([f'{line} | Level: {str(hp[1])}', 'HypixelHighLevel'])
                ...
                if self.mpl or self.mpr:
                    ...
                    if self.mpr:
                        if mp[0] != 'False':
                            ...
                            self.towrite.append([f'{line} | Rank: {mp[0]}', 'MineplexRanked'])
                            data += f'\nMineplexRank: {mp[0]}'
                        if self.mpl:
                            if int(mp[1]) != 0:
                                ...
                                if int(mp[1]) >= self.mpminl:
                                    ...
                                    self.towrite.append([f'{line} | Level: {str(mp[1])}', 'MineplexHighLevel'])

                if self.hivemcrank:
                    hivemc_rank = self.hivemc(uuid)
                    if hivemc_rank is not False:
                        self.towrite.append([f'{line} | Rank: {str(hivemc_rank)}', 'HiveRanked'])
                        Counter.hivemcrank += 1
                        data += f'\nHiveRank: {str(hivemc_rank)}'
                if self.minecon:
                    if self.mojang(uuid):
                        Counter.mojang += 1
                        self.towrite.append([f'{line} | Username: {username}', 'MineconCape'])
                        data += '\nMojangCape: True'
                if self.optifinecape:
                    if self.optifine(username):
                        Counter.optifine += 1
                        self.towrite.append([f'{line} | Username: {username}', 'OptifineCape'])
                        data += '\nOptifineCape: True'
                if self.labymodcape:
                    if self.labymod(uuid):
                        Counter.labymod += 1
                        self.towrite.append([f'{line} | Username: {username}', 'LabymodCape'])
                        data += '\nLabymodCape: True'
                if self.liquidcape:
                    if self.liquidbounce(uuid):
                        Counter.liquidbounce += 1
                        self.towrite.append([f'{line} | Username: {username}', 'LiquidBounceCape'])
    ...
    def writing(self):
        while True:
            while len(self.towrite) != 0:
                z = self.towrite[0]
                self.towrite.remove(z)
...
class Checker:
    ...
    class Level:
        ...
        mineplex_level = int(config['checker']['level']['hypixel_level'])
