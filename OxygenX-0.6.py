            ...
            try:
                ...
                if OxygenX.Proxy.proxy and not OxygenX.Proxy.API.use:
                    ...
                    loader = open(fileopenbox(title="Load Proxies List", default="*.txt"), 'r', encoding="utf8",
                                  errors='ignore').read().split('\n')
                elif OxygenX.Proxy.API.use and OxygenX.Proxy.proxy:
                    try:
                        idk = False
                        ...
                        sleep(60)
                        break
                self.proxylist = [x.strip() for x in loader if ":" in x]
...
if __name__ == '__main__':
    ...
    charz = ['@', '!', '#', '$', '%', '^', '&', '*', ')', '(', '-', '}', '{', ']', '"', '+', '=', '?', '/',
             '.', '>', ',', '<', '`', '\'', '~', '[', '\\']
    t = f'''{Fore.LIGHTCYAN_EX}________                                     ____  ___
\_____  \ ___  ______.__. ____   ____   ____ \   \/  /
 /   |   \\\  \/  <   |  |/ ___\_/ __ \ /    \ \     /
/    |    \>    < \___  / /_/  >  ___/|   |  \/     \\
\_______  /__/\_ \/ ____\___  / \___  >___|  /___/\  \\
